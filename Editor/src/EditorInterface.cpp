...
static void InspectProgram(GLuint program)
{
...
void EditorInterface::OnTick()
{
    ...
    {
		InspectProgram(engine->GetScene().renderSystem->GetMainShader());
#if 0
        static bool asColor = false;
        ImGui::Checkbox("As color", &asColor);

        GLuint program = engine->GetScene().renderSystem->GetMainShader();
        glUseProgram(program);
        GLint uniformCount;
        ...
        for (int index = 0; index < uniformCount; ++index)
        {
            ...
            if (type == GL_FLOAT_VEC3)
            {
                GLfloat param[3];
                glGetUniformfv(program, location, param);
                if (asColor ? ImGui::ColorEdit3(name, param) : ImGui::DragFloat3(name, param))
